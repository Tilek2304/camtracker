...
PID_DT = 1.0 / PID_HZ
ALERT_PERIOD = timedelta(seconds=30)    # каждые 30с при видимом объекте
DET_VISIBLE_TIMEOUT = 0.5               # объект считается видимым 0.5с после последней детекции
...
# Аудио через VLC
MUSIC_FILE = "/path/to/audio.mp3"   # <-- УКАЖИ ПУТЬ К MP3
...
def start_alert(vlc_proc_ref):
    ...
    try:
        p = subprocess.Popen(['cvlc', '-I', 'dummy', '--play-and-exit', MUSIC_FILE],
